              ...
              {messages.map((message, index) => (
                ...
                >
                  <div className="relative z-10 flex flex-col md:flex-row w-full h-full  backdrop-blur-sm">
                    {/* Text Section */}
                    <div className="md:w-1/2 flex flex-col justify-center items-start p-24 lg:ml-12 md:ml-32 dark:text-white text-black">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-14">
              {gameServers.map(gameServer =>
                <Link href={`/gameserver/${gameServer.id}`} key={gameServer.id}>
                  {/* <div className='flex min-h-[50vh] flex-col w-[16vw] rounded border mb-20 border-neutral-700 gap-4'>
                  <div className='h-[30vh] rounded border-neutral-700 relative overflow-hidden  m-4 '>
                    <Image
                      src={gameServer.image}
                      alt={gameServer.id}
                      layout="fill"
                      className="object-cover w-full h-full transition-all duration-300 transform group-hover:scale-105 group-hover:brightness-10"
                    />
                  </div>
